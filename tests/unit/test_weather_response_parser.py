...
def test_should_parse_weather_response_into_records(valid_weather_response):
    records = WeatherResponseParser.parse(valid_weather_response)
    assert len(records) == 3


def test_should_parse_first_weather_record_correctly(valid_weather_response):
    records = WeatherResponseParser.parse(valid_weather_response)
...
def test_should_parse_dt_base_as_date(valid_weather_response):
    records = WeatherResponseParser.parse(valid_weather_response)

    assert records[0]["dt_base"] == date(2018, 1, 1)
    assert isinstance(records[0]["dt_base"], date)


def test_should_preserve_weather_coordinates(valid_weather_response):
    records = WeatherResponseParser.parse(valid_weather_response)
...
def test_should_accept_missing_optional_metadata(valid_weather_response):
...
@pytest.mark.parametrize(
    "missing_field",
    ["latitude", "longitude", "timezone", "daily"],
)
def test_should_reject_missing_required_root_field(
    ...
    with pytest.raises(
        ...
        WeatherResponseParser.parse(valid_weather_response)


def test_should_reject_non_dictionary_daily(valid_weather_response):
...
def test_should_reject_missing_required_daily_field(
    ...
    valid_weather_response["daily"].pop(missing_field)
...
def test_should_reject_non_list_daily_field(valid_weather_response):
    valid_weather_response["daily"]["rain_sum"] = "1.6"
...
def test_should_reject_different_daily_field_lengths(valid_weather_response):
    valid_weather_response["daily"]["rain_sum"] = [1.6, 1.1]
...
def test_should_accept_empty_daily_arrays(valid_weather_response):
    for field in WeatherResponseParser.REQUIRED_DAILY_FIELDS:
...
def test_should_reject_invalid_daily_date(valid_weather_response):
    valid_weather_response["daily"]["time"][0] = "2018-02-31"
