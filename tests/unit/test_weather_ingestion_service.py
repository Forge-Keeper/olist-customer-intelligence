...
def weather_response() -> dict:
    return {
        ...
        "daily": {
            ...
            "temperature_2m_max": [25.7, 25.2],
            "temperature_2m_min": [19.9, 20.1],
            "rain_sum": [1.6, 1.1],
            "wind_speed_10m_max": [20.2, 28.8],
        },
    }
...
def parsed_records() -> list[dict]:
    return [
        {"dt_base": date(2018, 1, 1), "temperature_2m_mean": 22.5},
        {"dt_base": date(2018, 1, 2), "temperature_2m_mean": 22.2},
    ]
...
def test_should_forward_custom_daily_variables(
    ...
    client.get_historical_weather.return_value = weather_response
    variables = ["temperature_2m_mean", "rain_sum"]
...
def test_should_parse_api_response(
    ...
    mock_parse.assert_called_once_with(weather_response)
...
def test_should_use_same_request_id_for_raw_and_bronze(
    ...
    raw_request_id = raw_writer.write.call_args.kwargs["request_id"]
    bronze_request_id = bronze_writer.write.call_args.kwargs["request_id"]
...
def test_should_not_write_bronze_when_parser_fails(
    ...
    client.get_historical_weather.return_value = weather_response
    mock_parse.side_effect = ValueError("Invalid weather response")
...
def test_should_stop_pipeline_when_raw_write_fails(
    ...
    client.get_historical_weather.return_value = weather_response
    raw_writer.write.side_effect = RuntimeError("RAW write failed")
