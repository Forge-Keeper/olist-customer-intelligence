...
from olist_data_platform.domains.ingestion.ibge.datasets import (
    MUNICIPALITY_BUSINESS_ACTIVITY,
)
...
def test_service_partitions_requests_by_period_and_variable() -> None:
    writer = _Writer()
    client = _SidraClient()
    service = cempre_service.MunicipalityBusinessActivityIngestionService(
        client=client,
        bronze_writer=writer,
        request_id_factory=lambda: "req-cempre",
    )

    request_id = service.ingest()

    assert request_id == "req-cempre"
    assert len(client.slices) == 24
    assert client.slices[0] == (("2016",), ("367",))
    assert client.slices[-1] == (("2018",), ("10143",))
    assert len(writer.records) == 24
