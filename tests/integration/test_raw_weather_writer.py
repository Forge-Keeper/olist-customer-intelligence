...
def test_should_build_single_row_raw_dataframe(
    spark: SparkSession,
...
def test_should_preserve_request_metadata(
    spark: SparkSession,
    weather_response: dict,
) -> None:
    writer = RawWeatherWriter(
...
def test_should_serialize_original_response_as_json(
    spark: SparkSession,
    weather_response: dict,
) -> None:
    writer = RawWeatherWriter(
    ...
    deserialized_response = json.loads(
        row.response_json
    )

    assert deserialized_response == weather_response
...
def test_should_preserve_complete_nested_response(
    spark: SparkSession,
    weather_response: dict,
) -> None:
    writer = RawWeatherWriter(
    ...
    assert response["daily"]["rain_sum"] == [
        1.6,
        1.1,
        3.4,
    ]

    assert response["daily_units"][
        "temperature_2m_mean"
    ] == "°C"
