...
def test_should_build_real_bronze_dataframe(spark, weather_records):
...
def test_should_build_dataframe_with_expected_schema(spark, weather_records):
    writer = BronzeWeatherWriter(
        spark=spark,
        target_table="bronze.weather_daily",
    )
...
def test_should_preserve_weather_record_values(spark, weather_records):
    writer = BronzeWeatherWriter(
        spark=spark,
        target_table="bronze.weather_daily",
    )
    ...
    assert first_row.dt_base == date(2018, 1, 1)
...
def test_should_add_request_metadata_to_every_record(spark, weather_records):
    writer = BronzeWeatherWriter(
        spark=spark,
        target_table="bronze.weather_daily",
    )

    rows = writer._build_dataframe(
        records=weather_records,
        request_id="request-123",
        requested_latitude=-23.5505,
        requested_longitude=-46.6333,
...
def test_should_generate_ingestion_timestamp(spark, weather_records):
    writer = BronzeWeatherWriter(
        spark=spark,
        target_table="bronze.weather_daily",
    )
...
def test_should_create_expected_spark_data_types(spark, weather_records):
    writer = BronzeWeatherWriter(
        spark=spark,
        target_table="bronze.weather_daily",
    )
    ...
    assert isinstance(schema["temperature_2m_mean"], DoubleType)
    assert isinstance(schema["utc_offset_seconds"], IntegerType)
    assert isinstance(schema["ingestion_timestamp"], TimestampType)


def test_should_keep_nullable_weather_values(spark):
    records = [
        {
            "dt_base": date(2018, 1, 1),
