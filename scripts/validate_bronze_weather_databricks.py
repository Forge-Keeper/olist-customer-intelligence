...
def validate_table(spark: SparkSession, target_table: str) -> None:
    ...
    if detail is None:
        raise AssertionError(
            "DESCRIBE DETAIL returned no metadata."
        )

    if detail["format"] != "delta":
        raise AssertionError(
            f"Expected Delta table, got {detail['format']!r}."
        )

    partition_columns = list(
        detail["partitionColumns"] or []
    )
    if partition_columns != EXPECTED_PARTITION_COLUMNS:
        raise AssertionError(
            "Unexpected partition columns: "
            f"expected={EXPECTED_PARTITION_COLUMNS}, "
            f"actual={partition_columns}"
        )

    clustering_columns = list(
        detail["clusteringColumns"] or []
    )
    if clustering_columns != EXPECTED_CLUSTERING_COLUMNS:
        raise AssertionError(
            "Unexpected clustering columns: "
            f"expected={EXPECTED_CLUSTERING_COLUMNS}, "
            f"actual={clustering_columns}"
        )
    ...
    if (
        ...
        raise AssertionError(
            "Bronze table contains NULL primary-key values."
        )
    ...
    if duplicate_count:
        raise AssertionError(
            "Bronze table contains duplicate primary keys."
        )
    ...
    print(
        "table_features="
        f"{list(detail['tableFeatures'] or [])}"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the Weather Bronze table on Databricks."
        )
    )
    parser.add_argument(
        "--target-table",
        required=True,
        help=(
            "Fully qualified Bronze table name, for example "
            "catalog.bronze.weather_daily."
        ),
    )
