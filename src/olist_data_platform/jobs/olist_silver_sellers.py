...
from olist_data_platform.domains.silver.olist.sellers import process_sellers_snapshot
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Transform Olist Bronze Sellers into Silver."
    )
...
def run(args: argparse.Namespace, spark: SparkSession) -> tuple[str, int]:
    ...
    try:
        report = process_sellers_snapshot(
            spark=spark,
...
def main() -> None:
    ...
    print(f"olist_silver_sellers_completed run_id={run_id} rows={row_count}")
