...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Load the Olist geolocation CSV snapshot into Bronze."
    )
...
def run(args: argparse.Namespace, spark: SparkSession) -> tuple[str, int]:
    ...
    reader = OlistCsvSnapshotReader(
        spark=spark,
        source_path=args.source_path,
        required_columns=GEOLOCATION_SOURCE_COLUMNS,
        dataset_name="olist_geolocation",
    )
    writer = BronzeWriter(
        spark=spark,
        target_table=args.target_table,
    ...
    service = OlistSnapshotIngestionService(
        ...
        quality_contract=OLIST_GEOLOCATION_QUALITY_CONTRACT,
...
def main() -> None:
    ...
    print(f"olist_geolocation_ingestion_completed run_id={run_id} rows={row_count}")
