...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Transform Olist Bronze Products into Silver."
    )
    parser.add_argument("--source-table", required=True)
    parser.add_argument("--translations-table", required=True)
...
def run(args: argparse.Namespace, spark: SparkSession) -> tuple[str, int]:
    ...
    execution_scope = json.dumps(
        {
            ...
            "translations_table": args.translations_table,
        },
        sort_keys=True,
        separators=(",", ":"),
    )
    ...
    try:
        report = process_products_snapshot(
            spark=spark,
            bronze=spark.table(args.source_table),
            translations=spark.table(args.translations_table),
            target_table=args.target_table,
...
def main() -> None:
    ...
    print(f"olist_silver_products_completed run_id={run_id} rows={row_count}")
