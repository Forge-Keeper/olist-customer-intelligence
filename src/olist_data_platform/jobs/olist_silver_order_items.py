...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Transform Olist Bronze Order Items into Silver."
    )
    parser.add_argument("--source-table", required=True)
    parser.add_argument("--orders-table", required=True)
    parser.add_argument("--products-table", required=True)
    parser.add_argument("--sellers-table", required=True)
...
def run(args: argparse.Namespace, spark: SparkSession) -> tuple[str, int]:
    ...
    execution_scope = json.dumps(
        {
            ...
            "sellers_table": args.sellers_table,
        },
        sort_keys=True,
        separators=(",", ":"),
    )
    ...
    try:
        report = process_order_items_snapshot(
            spark=spark,
            bronze=spark.table(args.source_table),
            orders=spark.table(args.orders_table),
            products=spark.table(args.products_table),
            sellers=spark.table(args.sellers_table),
            target_table=args.target_table,
...
def main() -> None:
    ...
    print(f"olist_silver_order_items_completed run_id={run_id} rows={row_count}")
