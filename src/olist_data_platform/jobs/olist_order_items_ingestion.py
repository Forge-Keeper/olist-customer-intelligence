...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Load the Olist order items CSV snapshot into Bronze."
    )
...
def run(args: argparse.Namespace, spark: SparkSession) -> tuple[str, int]:
    ...
    reader = OlistCsvSnapshotReader(
        spark=spark,
        source_path=args.source_path,
        required_columns=ORDER_ITEMS_SOURCE_COLUMNS,
        dataset_name="olist_order_items",
    )
    writer = BronzeWriter(
        spark=spark,
        target_table=args.target_table,
        config=OLIST_ORDER_ITEMS_BRONZE_CONFIG,
    )
    service = OlistSnapshotIngestionService(
        dataset_name="olist_order_items",
        reader=reader,
        bronze_writer=writer,
        quality_contract=OLIST_ORDER_ITEMS_QUALITY_CONTRACT,
...
def main() -> None:
    ...
    print(f"olist_order_items_ingestion_completed run_id={run_id} rows={row_count}")
