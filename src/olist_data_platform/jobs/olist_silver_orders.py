...
from olist_data_platform.domains.silver.olist.orders import process_orders_snapshot
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Transform Olist Bronze Orders into Silver."
    )
...
def run(args: argparse.Namespace, spark: SparkSession) -> tuple[str, int]:
    ...
    try:
        report = process_orders_snapshot(
            spark=spark,
            bronze=spark.table(args.source_table),
            customers=spark.table(args.customers_table),
            target_table=args.target_table,
...
def main() -> None:
    ...
    print(f"olist_silver_orders_completed run_id={run_id} rows={row_count}")
