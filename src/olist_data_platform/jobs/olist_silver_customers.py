...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Transform Olist Bronze Customers into Silver."
    )
...
def run(args: argparse.Namespace, spark: SparkSession) -> tuple[str, int]:
    ...
    try:
        report = process_customers_snapshot(
            spark=spark,
...
def main() -> None:
    ...
    print(f"olist_silver_customers_completed run_id={run_id} rows={row_count}")
