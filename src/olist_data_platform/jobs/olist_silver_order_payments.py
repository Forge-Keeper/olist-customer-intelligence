...
from olist_data_platform.domains.silver.olist.order_payments import (
    process_order_payments_snapshot,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Transform Olist Bronze Order Payments into Silver."
    )
...
def run(args: argparse.Namespace, spark: SparkSession) -> tuple[str, int]:
    ...
    try:
        report = process_order_payments_snapshot(
            spark=spark,
...
def main() -> None:
    ...
    print(f"olist_silver_order_payments_completed run_id={run_id} rows={row_count}")
