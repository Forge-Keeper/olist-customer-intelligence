...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Load the Olist order payments CSV snapshot into Bronze."
    )
...
def run(args: argparse.Namespace, spark: SparkSession) -> tuple[str, int]:
    ...
    reader = OlistCsvSnapshotReader(
        spark=spark,
        source_path=args.source_path,
        required_columns=ORDER_PAYMENTS_SOURCE_COLUMNS,
        dataset_name="olist_order_payments",
    )
    writer = BronzeWriter(
        spark=spark,
        target_table=args.target_table,
        config=OLIST_ORDER_PAYMENTS_BRONZE_CONFIG,
    )
    service = OlistSnapshotIngestionService(
        dataset_name="olist_order_payments",
        reader=reader,
        bronze_writer=writer,
        quality_contract=OLIST_ORDER_PAYMENTS_QUALITY_CONTRACT,
...
def main() -> None:
    ...
    print(
        f"olist_order_payments_ingestion_completed run_id={run_id} rows={row_count}"
    )
