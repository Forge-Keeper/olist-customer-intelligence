...
| Olist Product Category Name Translation Silver | ✅ | ✅ | ? | ? | second Silver slice accepted in DEV: 71 rows / 71 distinct source categories, zero null category names, deterministic rerun |
| Olist Products Silver | ✅ | ✅ | ? | ? | second Silver slice accepted in DEV: 32,951 rows / 32,951 distinct `product_id`; typed product measures; 13 untranslated non-null categories and 4 zero-weight products retained as non-blocking observations; deterministic rerun |
| Olist Sellers Silver | ✅ | ✅ | ? | ? | second Silver slice accepted in DEV: 3,095 rows / 3,095 distinct `seller_id`, deterministic rerun |
| Olist Order Items Silver | ✅ | ✅ | ? | ? | second Silver slice accepted in DEV: 112,650 rows / 112,650 distinct `(order_id, order_item_id)`, zero Orders/Products/Sellers orphans, typed timestamp/monetary fields, deterministic rerun; controlled Products-FK rejection persisted DQ and preserved the target |
...
Olist Bronze CSV code coverage is complete for the current public Olist source set represented by this repository. Olist Silver is now delivered in DEV for Customers, Orders, Product Category Translation, Products, Sellers and Order Items. Environment readiness is intentionally shown separately above rather than inferred from implementation alone.
...
| Silver typed contracts + protected full-snapshot writes | ✅ | ✅ | ? | ? | Customers, Orders, Category Translation, Products, Sellers and Order Items have accepted DEV evidence for explicit typed schemas, blocking/non-blocking DQ, deterministic reruns and protected target behavior |
| First-class Data Quality + administrative Control Plane | ✅ | ✅ | ✅ | ✅ | GDP, Customers and Sellers provide accepted cross-environment evidence; two Olist Silver slices add accepted DEV evidence; adoption still varies by dataset |
...
- explicit Silver contracts for Customers, Orders, Product Category Translation, Products, Sellers and Order Items with typed fields, referential DQ, lineage, deterministic `FULL_REPLACE` and protected writes;
- runtime-safe shared operational writes that validate existing control-plane table schema/layout without repeatedly reconciling metadata on the concurrent hot path;
...
- **Silver Products + Category + Sellers + Order Items DEV:** Category Translation contains 71 unique source categories; Products contains 32,951 unique products, including 13 untranslated non-null categories and 4 zero-weight observations retained as warnings; Sellers contains 3,095 unique sellers; Order Items contains 112,650 unique `(order_id, order_item_id)` rows with zero Orders/Products/Sellers orphans in the accepted snapshot. A semantic rerun returned `0/0` bidirectional `EXCEPT ALL` for all four datasets when excluding `silver_processed_timestamp`. A controlled validation parent produced 9 Product-FK orphans; `OLIST-SILVER-ORDER-ITEMS-DQ05` persisted `ERROR/FAIL` with `invalid_row_count=9`, the run raised `DataQualityRejectedError`, and the protected target remained unchanged (`0/0`).
...
- Silver delivery currently covers Customers, Orders, Product Category Translation, Products, Sellers and Order Items in DEV; Payments, Reviews, funnel/geography conformance and all Gold analytical products are not yet delivered in Silver/Gold;
