...
DEV runtime acceptance is complete for `dev.bronze.olist_order_items`.

First execution:

- Databricks run ID: `1081353954497639`;
- application run ID: `d2d5246a-811c-4f55-a31f-9d7bb31b700f`;
- status: `SUCCESS`;
- source rows read: `112,650`;
- persisted rows reported by the job: `112,650`;
- write strategy observed in runtime logs: `FULL_REPLACE`;
- target table created successfully.

Second execution (idempotence check):

- Databricks run ID: `349855951448120`;
- application run ID: `6a7a1100-88fd-4b52-8111-a9b75828dcb6`;
- status: `SUCCESS`;
- source rows read: `112,650`;
- persisted rows reported by the job: `112,650`;
- write strategy observed in runtime logs: `FULL_REPLACE`.
...
- rows: `112,650`;
- distinct (`order_id`, `order_item_id`) keys: `112,650`;
- null `source_file`: `0`;
...
DEV acceptance is complete for source profiling, composite-key evidence, source fidelity, timestamp/numeric parseability, managed metadata completeness and repeatable `FULL_REPLACE` behavior.
